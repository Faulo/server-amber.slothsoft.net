...
- Fixes in the Antique Area
  - The exit to the mines is now marked on the dungeon map
  - When exiting to the mines, the spawn direction is now correct
  - Fixed missing elevator platforms when going down and up again
  - In last room with the nav stone chests, when failing lockpicking,
    enemies spawn and a message shows. When doing it again while the
    enemies are still around, the event is not happening again until
    they are dead.
  - When crafting nav stones and opening the machine with the crafted
    stones but leaving it without taking the stone, it won't be destroyed
    anymore. So you can pick it up later.
- Fixed a visual bug for the SP stealer spell (green orb size)
- Nera's ring is no longer destroyed if you give it to Bralum instead
  of showing it only
- Some minor german text fixes:
  - Name of search trainer in gnome mine
  - Default welcome message for search trainers
  - Lady Heidi fairy response
  - Fast travel point name in S'Angrila
  - Second welcome text of Lebab
  - Look-at text of Nagier
- Fixed a bug in monster decision making when they determine the
  target row of a row spell. Now the back player row can also be
  targeted if the threat is greater than the front row threat.
- Random-valued rewards (e.g. wishing well attribute increases) will now
  be in the range 1 to max value instead of 0 to max value minus 1.
  So they work similar to potion effects (e.g. a strength potion) which
  use the same logic.
- When an ingame year passes, now the age of the character is no longer
