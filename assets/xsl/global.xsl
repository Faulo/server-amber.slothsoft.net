...
<xsl:stylesheet version="1.0" xmlns="http://www.w3.org/1999/xhtml" xmlns:html="http://www.w3.org/1999/xhtml" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:sfm="http://schema.slothsoft.net/farah/module" xmlns:sfd="http://schema.slothsoft.net/farah/dictionary"
    ...
    <xsl:variable name="sitemap" select="//sfs:domain" />
    <xsl:variable name="currentPage" select="$sitemap/descendant-or-self::*[@current]" />
    <xsl:variable name="ancestorPages" select="$currentPage/ancestor::sfs:*" />
    <xsl:variable name="childPages" select="$currentPage/sfs:page[@status-active and @status-public]" />
